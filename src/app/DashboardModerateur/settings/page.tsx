...
export default function ModerateurSettingPage() {
  ...
  useEffect(() => {
    ...
    if (Array.isArray(roles)) {
      // Si user.role est un tableau, vérifier que "MODERATEUR" est inclus
      if (!roles.includes("MODERATEUR")) {
        router.push("/unauthorized");
      }
    } else {
      // Si user.role est un simple string
      if (roles !== "MODERATEUR") {
        router.push("/unauthorized");
      }
    }
  ...
  const handleChange = (field: keyof ModeratorSettings, value: string) => {
  ...
  return (
    <div className="p-6 w-1/2">
      <Card className="w-full p-4 mx-auto">
        <CardHeader>
          <CardTitle>Paramètres Modérateur</CardTitle>
        </CardHeader>
        <CardContent>
